...
class Node:
            ...
            if new_puzzle.move(i):
                if self.parent:
                    if not self.parent.puzzle.equals(new_puzzle):
                        ...
                        print(
                            f"Cannot make that same move: {i}, position = ({self.puzzle.i}, {self.puzzle.j})"
                        )
                if not self.parent:
                    self.add_child(new_puzzle)
        return None
    ...
    def search_high_leaf_node_f(self, highest):
        if (
            self.is_leaf()
            and self.g() + self.puzzle.h() > highest.g() + highest.puzzle.h()
        ):
            return self
        if self.children:
            for child in self.children:
    ...
    def lower_or_equal_f_than(self, other) -> bool:
        return self.g() + self.puzzle.h() <= other.g() + other.puzzle.h()
    ...
    def print(self):
        ...
        print(
            f"{prefix} g(n) = {self.g()}, h(n) = {self.puzzle.h()}, f(n) = {f_current}, hoja={' SI' if self.is_leaf() else ' NO'} {'****SOLUCION*****' if self.puzzle.is_obj() else ''}"
        )
    ...
    def print_with_children(self):
        f_current = self.g() + self.puzzle.h()
        print(
            f"g(n) = {self.g()}, h(n) = {self.puzzle.h()}, f(n) = {f_current}, hoja={' SI' if self.is_leaf() else ' NO'} {'****SOLUCION*****' if self.puzzle.is_obj() else ''}"
        )
        print(self.puzzle.print())
        if self.children:
            for child in self.children:
